        ...
        if s["page_step"] == "setup_player_count":
            ...
            if s.get("current_question", "").startswith("لا توجد أسئلة"):
                ...
                if s["skip_chances_offline"] > 0:
                    ...
                    offline_main_column.controls.append(ft.Text("❌ لا يمكن تغيير السؤال.", size=16, color=colors.RED_ACCENT_700))
            ...
            if total_votes_cast > 0 and sorted_results and sorted_results[0][1] > 0:
                ...
                offline_main_column.controls.append(ft.Text(f"الأكثر حصولاً على الأصوات: {', '.join(most_voted_players)}", size=20, weight="bold", color=colors.PRIMARY))
            else:
...
def min_fina_online_logic(page: ft.Page, go_home_fn, send_action_fn, room_code: str, current_player_name: str, game_rooms_ref):
        ...
        if current_phase == "LOBBY":
            ...
            if is_host:
                ...
                if not can_host_start:
                    ...
                    if needed > 0 :
                         action_area.controls.append(ft.Text(f"تحتاج لـ {needed} لاعبين إضافيين للبدء.", color=colors.ORANGE_ACCENT_700, text_align=ft.TextAlign.CENTER))
                    elif current_player_count < min_players_to_start:
                         action_area.controls.append(ft.Text(f"تحتاج لـ {min_players_to_start - current_player_count} لاعبين إضافيين كحد أدنى.", color=colors.RED_700, text_align=ft.TextAlign.CENTER))
            else:
            ...
            if gs.get("current_question","").startswith("لا توجد أسئلة"):
                ...
                if gs.get("skip_chances_left",0) > 0:
                    ...
                    action_area.controls.append(ft.Text("❌ لا يمكن تغيير السؤال.", size=16, color=colors.RED_ACCENT_700))
            ...
            if total_votes_cast_results > 0 and sorted_vote_results and sorted_vote_results[0][1] > 0:
                ...
                action_area.controls.append(ft.Text(f"الأكثر حصولاً على الأصوات: {', '.join(most_voted_players_res)}", size=20, weight="bold", color=colors.PRIMARY, text_align=ft.TextAlign.CENTER))
            else:
    ...
    online_main_container.controls.extend([
        ...
        ft.Row([
            ft.Container(content=player_list_display, padding=10, border=ft.border.all(1, colors.OUTLINE), border_radius=5, width=250, margin=ft.margin.only(right=10)),
            ft.VerticalDivider(),
